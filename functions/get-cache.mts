...
import pkg1 from '@googlemaps/url-signature';
// Import JWT module
import pkg2, { VerifyOptions } from 'jsonwebtoken';
	...
	async (event: HandlerEvent): Promise<HandlerResponse> => {
		...
		return new Promise<false | GeoScoutToken>((resolve, reject) => {
			...
			.then((decodedToken) => {
				...
				return client
					.api(
						`/sites/${siteId}/lists/${listId}/items?$expand=fields($select=Title,W3WLocation,Coordinates,Found,Suspended,Polygon,Difficulty)&$select=id,fields&$filter=fields/Title eq '${cacheId}'`,
					)
