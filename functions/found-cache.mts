...
export default withLambda(
	async (event: HandlerEvent): Promise<HandlerResponse> => {
		...
		try {
			...
			cacheCode = JSON.parse(event.body as string).cacheCode;
		...
		const uniqueToken = crypto
			.createHash('SHA256')
			.update(
				`${event.headers['x-nf-client-connection-ip'] || event.headers['client-ip']}-${cacheId}`,
			)
			.digest('hex');
		try {
		...
		return new Promise<GeoScoutToken>((resolve) => {
			resolve(
				verify(accessToken, jwtSecret, jwtVerifyOptions) as GeoScoutToken,
			);
		})
			.then((decodedToken) => {
				...
				return (
					client
						.api(
							`/sites/${siteId}/lists/${listId}/items?$expand=fields($select=Title,CableTieCode,Found)&$select=id,fields&$filter=fields/Title eq '${cacheId}'`,
						)
			...
			.then((data: { value: SharePointCacheRecord[] }) => {
				...
				if (cacheRecord) {
					...
					return client
						.api(
							`/sites/${siteId}/lists/${userListId}/items/${recordId}?$expand=fields($select=Title,FoundCaches,Total,Username)&$select=id,fields`,
						)
						.header('Prefer', 'allowthrottleablequeries')
						.get();
				} else {
			...
			.then((data) => {
				if (data.hasOwnProperty('fields')) {
					...
					if (tokenIds.find((id) => id === tokenId)) {
						...
						return client
							.api(
								`/sites/${siteId}/lists/${userListId}/items/${recordId}/fields`,
							)
							.patch({
			...
			.then(() => {
				return client
					.api(
						`/sites/${siteId}/lists/${listId}/items/${currentStats.id}/fields`,
					)
					.patch({
				...
				if (error === 'Invalid code') {
					...
					return {
						...
						body: JSON.stringify({
							...
							errorDebug:
								'Token not found in backend - contact support@geoscout.uk',
						}),
						headers,
					};
					...
					return {
						...
						body: JSON.stringify({
							...
							errorDebug:
								'Your User ID has already found this cache - contact support@geoscout.uk if you believe this is incorrect',
						}),
						headers,
					};
				} else {
